...
let rec diff ((exp : aexp), (x : string)) : aexp =
  ...
  | Times lst ->
      let rec timedf (lst : aexp list) (ans : aexp list) : aexp list =
        match lst with
        | [] -> ans
        | hd :: tl ->
            let rec isvar (lst : aexp list) : bool =
              match lst with
              | [] -> false
              | hd :: tl -> (
                  match hd with
      ...
      Times (timedf lst [])
  | Sum lst ->
      let rec sumdf (lst : aexp list) (ans : aexp list) : aexp list =
...
let (_ : aexp) = diff (Times [ Const 5; Power ("x", 3) ], "x")
