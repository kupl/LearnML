...
let rec diff ((e : aexp), (str : string)) : aexp =
  match e with
  | Const i -> Const 0
  | Var str1 -> if str1 = str then Const 1 else Const 0
  | Power (str1, i) ->
      if str1 = str then
        if i != 0 then Times [ Const i; Power (str1, i - 1) ] else Const 0
      else Const 0
  | Sum [] -> raise InvalidArgument
  | Sum lst ->
      if List.length lst = 1 then diff (List.hd lst, str)
  ...
  | Times lst ->
      if List.length lst = 1 then diff (List.hd lst, str)
        ...
        Sum
          [
            Times (diff (List.hd lst, str) :: List.tl lst);
