...
let rec checkpow (lst : aexp list) (str : string) : int =
  match lst with
  | [] -> 1
  | hd :: tl -> (
      match hd with
...
let checktemp1 (lst : aexp list) : aexp =
  match lst with [ Const t ] -> Const t | _ -> Times lst


let checktemp2 (lst : aexp list) : aexp =
  match lst with [ hd ] -> hd | [] -> Const 0 | _ -> Sum lst


let rec checktime (lst : aexp list) (str : string)
    (func : aexp list -> string -> aexp list) : aexp list =
  match lst with
  | [] -> []
  | hd :: tl -> (
      match hd with
...
let rec checksum (lst : aexp list) (str : string) : aexp list =
  match lst with
  | [] -> []
  | hd :: tl -> (
      match hd with
...
let (_ : aexp) = diff (Sum [ Times [ Const 2; Power ("y", 3) ]; Const 5 ], "y")

let (_ : aexp) = diff (Const 254, "x")

let (_ : aexp) = diff (Var "y", "x")

let (_ : aexp) = diff (Var "x", "x")

let (_ : aexp) = diff (Times [ Const 2; Var "x" ], "x")

let (_ : aexp) = diff (Sum [ Var "x"; Const 5 ], "x")
