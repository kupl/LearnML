...
let rec diff ((exp : aexp), (var : string)) : aexp =
  ...
  | Power (v1, n), v ->
      if v1 = v then Times [ Const n; Power (v1, n - 1) ] else Const 0
  | Times l, v -> (
      match l with
...
and iter (x : string) (l : aexp list) : int =
  match l with [] -> 0 | hd :: tl -> count x hd + iter x tl
