...
let rec diff (e, v) =
  ...
  | Times al ->
      ...
      let rec clean_times rs coef =
        match rs with
        | [] -> ([], coef)
        | Const n :: tl -> clean_times tl (coef * n)
        | hd :: tl ->
            let t0, m = clean_times tl coef in
            (hd :: t0, m)
      in

      let rec concat_times ts =
        match ts with
        | [] -> []
