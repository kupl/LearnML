...
let andAlso (x, y) = if x = true && y = true then true else false

let orElse (x, y) = if x = false && y = false then false else true
...
let rec equal (x, y) = if x = y then true else false
