...
let rec __s58 ((__s59 : aexp), (__s60 : string)) : aexp =
  match __s59 with
  | Const __s61 -> Const 0
  | Var __s62 -> if __s62 = __s60 then Const 1 else Const 0
  | Power (__s63, __s64) ->
      if __s63 = __s60 then Times [ Const __s64; Power (__s63, __s64 - 1) ]
      else Const 0
  | Times [] -> Const 0
  | Times (Const 0) :: __s65 -> Const 0
  | Times __s66 :: __s67 ->
      Sum
        [
          Times (__s58 (__s66, __s60) :: __s67);
          Times [ __s66; __s58 (Times __s67, __s60) ];
        ]
  | Sum __s68 ->
      Sum (List.map (fun (__s69 : aexp) -> __s58 (__s69, __s60)) __s68)
...
let diff ((exp : aexp), (x : string)) : aexp =
  ...
  match exp with
  | Times [] -> raise Failure "Exception(Template)"
  | Sum [] -> raise Failure "Exception(Template)"
  | _ -> __s58 (exp, x)
