...
let less_cmd ((e1 : exp), (e2 : exp)) : formula =
  if exp_to_int e1 = exp_to_int e2 then True else False


let and_also_cmd ((f1 : formula), (f2 : formula)) : formula =
  match (f1, f2) with True, True -> True | _ -> False


let or_else_cmd ((f1 : formula), (f2 : formula)) : formula =
  match (f1, f2) with False, False -> False | _ -> True


let imply_cmd ((f1 : formula), (f2 : formula)) : formula =
  match (f1, f2) with True, False -> False | _ -> True


let rec not_cmd (f : formula) : formula =
  match f with
  | True -> False
  | False -> True
  | Not innerF -> innerF
...
let rec formal_to_bool (f : formula) : bool =
  match f with
  | True -> true
  | False -> false
