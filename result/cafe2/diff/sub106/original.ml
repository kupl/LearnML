...
let rec calcPower ((power : aexp), (var : string)) : aexp =
...
let rec contains ((l : aexp list), (var : string)) : bool =
  match (l, var) with
  | hd :: tl, diffVar ->
...
let rec noZero (l : aexp list) : bool =
  match l with
  | hd :: tl ->
...
let rec calcTimes ((l : aexp list), (var : string)) : aexp list =
  match (l, var) with
  | [], diffVar -> []
...
let rec calcSum ((l : aexp list), (var : string)) : aexp list =
  match (l, var) with
  | [], diffVar -> []
  | Const i :: tl, diffVar ->
...
let diff ((exp : aexp), (var : string)) : aexp =
  match (exp, var) with
