...
= fun (exp, x) -> match exp with
  ...
  | Times lst -> let rec timedf : aexp list -> aexp list -> aexp list = fun lst ans -> match lst with
    | [] -> ans
    | hd::tl -> let rec isvar : aexp list -> bool = fun lst -> match lst with
      | [] -> false
      | hd::tl -> match hd with
      ...
      in if isvar lst=true then match hd with
        | Const n -> timedf tl (ans@[Const n])
        | _ -> timedf tl (ans@[diff (hd,x)])
        else ans@[Const 0]
    in Times (timedf lst [])
  | Sum lst -> let rec sumdf : aexp list -> aexp list -> aexp list = fun lst ans -> match lst with
    | [] -> ans
    | hd::tl -> sumdf tl ans@[diff (hd,x)]
    in Sum (sumdf lst []);;
    
diff (Sum [Power ("x",2); Times [Const 2; Var "x"]; Const 1], "x");;
diff (Sum [Times [Const 5; Power ("x",3)]; Times [Const 2; Var "x"]; Const 3],"x");;
diff (Times [Const 5; Power("x",3)],"x");;