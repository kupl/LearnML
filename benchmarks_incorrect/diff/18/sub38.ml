...
= fun (exp, x) -> 
  match (exp,x) with
    ...
    | (Power (a,b), x) -> if a=x then Times [Const b ;Power (a, b-1)] else Power(a,b)
  ...
  = fun li x -> 
    match li with
    |[] -> []
    |h::t ->  if t<>[] then [( Times ( [(diff (h,x))] @ t ) )]  @ [(Times  ( [h] @ (diff_time t x) ) )]
              else [( Times ( [(diff (h,x))] @ t ) )]
(*      if h = Var x then [diff (h, x)] @ diff_time t x else [h] @ diff_time t x*)
