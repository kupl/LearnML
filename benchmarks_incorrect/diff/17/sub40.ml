...
|Times lst -> if(let rec search lst x = match lst with
  |hd::tl -> (match hd with
    |Power(a,i) -> 1 + search tl x
    |Var(a) -> if a = x then 1 else 0 + search tl x
    |_ -> search tl x)
  |_ -> 0 in search lst x) = 0 then Const(0) else Times(let rec yes lst x  = match lst with
    |hd::tl -> (match hd with
    ...
    |_ -> [] in yes lst x)
|Sum lst -> Sum(let rec sumdiff lst x = match lst with
