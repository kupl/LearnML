...
= fun pgm cpgm input -> 
	...
	let psi_c = Executor.run cpgm input in
	print_endline ("Execution C : " ^ string_of_float (Unix.gettimeofday () -. temp_time));
	if BatSet.is_empty psi_c then
		None
	else (
		...
		let temp_time = Unix.gettimeofday () in
		let psi_b = Executor.run pgm input in
		let psi_b = BatSet.filter (fun (pc, sv) -> 
			let formula = [(Bool true, flatten_pc pc)] in
			match Z3_solve.is_valid ctor_table formula with
			| Some _ -> true
			| None -> false
		) psi_b
		in
		print_endline ("Execution B : " ^ string_of_float (Unix.gettimeofday () -. temp_time));
		Print.print_header "Buggy"; print psi_b;
		print_endline ("=======================\nSize : " ^ string_of_int (BatSet.cardinal psi_b));
		let temp_time = Unix.gettimeofday () in
		let vc = (VC_generator.run ctor_table psi_b psi_c) in
		print_endline ("VC time : " ^ string_of_float (Unix.gettimeofday () -. temp_time));
		print_endline ("VC Size : " ^ string_of_int (List.length vc));
	  Print.print_header "VC"; VC_generator.print_vc vc;
	  (*
	  let models = tapply (fun formula -> Z3_solve.check ctor_table formula) vc in
	  try List.find (fun model -> not (model = None)) models with _ -> None
	  get_model ctor_table vc
		Z3_solve.check2 ctor_table vc
		*)
		Z3_solve.check2 ctor_table vc
		)

let rec run3 : prog -> prog -> lexp list -> Z3.Model.model option
= fun pgm cpgm input -> 
	let ctor_table = Z3_solve.CtorTable.generation BatMap.empty pgm in
	let ctor_table = Z3_solve.CtorTable.generation ctor_table cpgm in
	(* Symbolic Formula Generation *)
	...
	if BatSet.is_empty psi_c then
		...
		let psi_c = BatSet.filter (fun (pc, sv) -> 
			let formula = [(Bool true, flatten_pc pc)] in
			match Z3_solve.is_valid ctor_table formula with
			| Some _ -> true
			| None -> false
		...
		print_endline ("=======================\nSize : " ^ string_of_int (BatSet.cardinal psi_b));
		let temp_time = Unix.gettimeofday () in
		let vc = (VC_generator.run2 ctor_table psi_b psi_c) in
		print_endline ("VC time : " ^ string_of_float (Unix.gettimeofday () -. temp_time));
		print_endline ("VC Size : " ^ string_of_int (List.length vc));
	  Print.print_header "VC"; VC_generator.print_vc vc;
		Z3_solve.check2 ctor_table vc
...
let rec run4 : prog -> prog -> lexp list -> Z3.Model.model option
= fun pgm cpgm input -> 
	let ctor_table = Z3_solve.CtorTable.generation BatMap.empty pgm in
	let ctor_table = Z3_solve.CtorTable.generation ctor_table cpgm in
	(* Symbolic Formula Generation *)
