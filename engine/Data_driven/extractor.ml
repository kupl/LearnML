...
let rec extract : t -> lexp -> t * lexp
= fun t (l, exp) -> 
  match exp with  
  | MINUS e ->
    let (t, e) = extract t e in
    (t, (l, MINUS e))
  | NOT  e ->
    let (t, e) = extract t e in
    (t, (l, NOT e))  
  | Raise e -> 
    ...
    (t, (l, Raise e))
  | ADD (e1, e2) ->
    let (t, (e1, e2)) = extract_tuple t (e1, e2) in
    (t, (l, ADD (e1, e2)))
  | SUB (e1, e2) ->
    let (t, (e1, e2)) = extract_tuple t (e1, e2) in
    (t, (l, SUB (e1, e2)))
  ...
  | ECtor (x, es) ->
    let (t, es) = extract_list t es in
    (t, (l, ECtor (x, es)))
  | EFun (arg, e) -> extract t e 
  | ELet (f, is_rec, args, typ, e1, e2) ->
    let (t, e1) = extract t e1 in
    if args <> [] || is_fun typ then 
      begin match f with
      | BindOne f -> 
        let (t, e2) = extract (BatMap.add f e1 t) e2 in
        (t, e2)
      | _ -> raise (Failure "Extract error : invalid function format")
      end
    else 
      let (t, e2) = extract t e2 in
  ...
  | IF (e1, e2, e3) ->
    let (t, e1) = extract t e1 in
    let (t, e2) = extract t e2 in
    let (t, e3) = extract t e3 in
    (t, (l, IF (e1, e2, e3)))
  | _ -> (t, (l, exp))

and extract_tuple : t -> (lexp * lexp) -> t * (lexp * lexp)
= fun t (e1, e2) ->
  let (t, e1) = extract t e1 in
  let (t, e2) = extract t e2 in
  (t, (e1, e2))

and extract_list : t -> lexp list -> t * lexp list
= fun t es ->
  match es with
  | [] -> (t, [])
  | hd::tl ->
    let (t, e) = extract t hd in
    let (t, es') = extract_list t tl in 
    (t, e::es')
