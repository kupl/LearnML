...
let rec gen_score_map : (string * 'a) list -> (string * 'a) list -> ((string * string) * float) list 
= fun ts1 ts2 ->
  match ts1 with
  | [] -> []
  | (s,v)::t ->
...
let padding : (string * 'a) list -> (string * 'a) list -> (string * 'a) list * (string * 'a) list
= fun ts1 ts2 ->
  let len1 = List.length ts1 in
  let len2 = List.length ts2 in
...
let calculate_mapping_distance : (string * 'a) list -> (string * 'a) list -> (string * string) list * float 
= fun ts1 ts2 ->
...
= fun topk sub solutions ->
  let vectorize = F.funcs_vectorize in
...
= fun topk sub solutions ->
  let vectorize = P.funcs_vectorize in
  let calculate = calculate_mapping_distance in
  let preproc = (fun x -> x |>  Extractor.extract_func_all |> vectorize) in
  let v_sub = preproc sub in
  let topk_lst = List.map (fun (f, sol) -> (f, sol, (preproc sol))) solutions |>
