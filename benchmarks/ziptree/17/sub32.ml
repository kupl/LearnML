...
let goUp loc = match loc with
    ...
    | LOC(t,HAND(left,upzipper,right)) -> LOC(NODE(List.append (List.rev left) (List.append [t] right) ) ,upzipper)

let goDown loc = match loc with
    | LOC(LEAF(leaf),_) -> raise(NOMOVE "down of leaf")
    | LOC(NODE([]),_) -> raise(NOMOVE "down is  empty")
