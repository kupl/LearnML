(*
    Homework 2, Exercise 5
    2015-15894 Jonghoon Won
    Sep 28, 2017
*)

type item = string

type tree = LEAF of item
          | NODE of tree list

type zipper = TOP
            | HAND of tree list * zipper * tree list

type location = LOC of tree * zipper

exception NOMOVE of string

let goLeft loc = match loc with
      LOC(t, TOP) -> raise (NOMOVE "left of top")
    | LOC(t, HAND(l::left, up, right)) -> LOC(l, HAND(left, up, t::right))
    | LOC(t, HAND([],up,right)) -> raise (NOMOVE "left of first")

let goRight: location -> location = fun (loc) ->
    match loc with
          LOC(_, TOP) -> raise (NOMOVE "right of top")
        | LOC(t, HAND(_, _, [])) -> raise (NOMOVE "right of last")
        | LOC(t, HAND(left, up, r::right)) -> LOC(r, HAND(t::left, up, right))

let goUp: location -> location = fun (loc) ->
    match loc with
          LOC(_, TOP) -> raise (NOMOVE "up of top")
        | LOC(t, HAND(left, up, right)) -> (
            let rec appendList: tree list * tree list -> tree list = fun (lst1, lst2) ->
                match lst1 with
                      [] -> lst2
                    | hd::tl -> appendList (tl, hd::lst2)
            in
            let appendedTree = appendList (left, t::right) in
            LOC(NODE(appendedTree), up)
            )

let goDown: location -> location = fun (loc) ->
    match loc with
          LOC(LEAF(_), _) -> raise (NOMOVE "down of bottom")
        | LOC(NODE([]), _) -> raise (NOMOVE "no child")
        | LOC(NODE(hd::tl), zip) -> LOC(hd, HAND([], zip, tl))
