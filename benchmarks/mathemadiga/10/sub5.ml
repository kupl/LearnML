...
exception SigmaDomainError of string
let rec value(exp, x, x_check) =
    match exp with
      X ->
        if x_check = 0 then raise(FreevarError "X is free value")
        else x
    ...
    | DIV(exp1, exp2) ->
      if value(exp2, x, x_check) = 0. then raise(DividedByZero "cannot divided by zero")
      else value(exp1, x, x_check) /. value(exp2, x, x_check)
    | SIGMA(exp1, exp2, exp3) ->
      let rec sigma_fun(exp3, a, b) =
        if a > b then raise(SigmaDomainError "check domain of sigma")
