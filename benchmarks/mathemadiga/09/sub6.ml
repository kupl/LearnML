...
let mathmadiga expr = 
	let rec calc( expr, env ) =
		let rec sigma( a, b, expr, env ) = if( a > b ) then 0.
							else calc( expr, a::env ) +. sigma( a +. 1., b, expr, env ) in
		let rec integral( a, b, expr, env ) = if( a > b ) then 0.
		...
		match expr with
			...
			| SIGMA ( a, b, c ) -> if( calc( a, env ) > calc( b, env ) ) then raise InvalidSigma
									else sigma( calc( a, env ), calc( b, env ), c, env )
			| INTEGRAL ( a, b, c ) -> if ( a > b ) 
...
print_float ( mathmadiga( INTEGRAL ( INT 1, INT 0, X) ) );;