		...
		match ex with
			...
			| SIGMA ( ex1, ex2, ex3 ) ->
				( let lb = evalX ( x, ex1 ) in
				  ...
				  if lb > ub then raise InvalidSigma
				  else if lb = ub || ( ub -. lb ) < 1.0 then evalX ( lb, ex3 )
				  else ( evalX ( lb, ex3 ) ) +. ( evalX ( x, ( SIGMA ( ( ADD ( ( REAL 1. ), ex1 ) ), ex2, ex3 ) ) ) ) ) 
			| INTEGRAL ( ex1, ex2, ex3 ) ->
				( let lb = evalX ( x, ex1 ) in
				  let ub = evalX ( x, ex2 ) in
				  if lb > ub then  -.  ( evalX ( x, ( INTEGRAL ( ex2, ex1, ex3 ) ) ) )
				  else if lb = ub then 0.
				  else ( ( evalX ( lb, ex3 ) ) *. 0.1 ) +. ( evalX ( x, ( INTEGRAL ( ( ADD ( ( REAL 0.1 ), ex1 ) ), ex2, ex3 ) ) ) ) ) in
