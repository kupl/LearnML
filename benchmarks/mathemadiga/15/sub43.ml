...
let rec substitute_x : exp * exp -> exp = fun(e, f) -> 
	...
	| SIGMA(e1, e2, e3) -> SIGMA(substitute_x (e, e1), substitute_x (e, e2), substitute_x (e, e3))
	| INTEGRAL(e1, e2, e3) -> INTEGRAL(substitute_x (e, e1), substitute_x (e, e2), substitute_x (e, e3))
...
let rec galculator e =
	...
	| SIGMA(e1, e2, e3) ->
		...
		else if(galculator e1 = galculator e2) then galculator(substitute_x (e1, e3))
		else
			galculator(substitute_x(e1, e3)) +.
			galculator(SIGMA(INT(int_of_float (galculator e1) + 1), e2, e3))
	| INTEGRAL(e1, e2, e3) ->
		if (galculator e1 = galculator e2) then 0.0
