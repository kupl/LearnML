...
let rec galculator (e: exp): float =
	...
	| SIGMA (a, b, f) -> (
		if (a > b) then 0.0
			...
			let (extended_list : exp) =
				if (List.length expr_list == 0) then REAL 0.0
				else if (List.length expr_list == 1) then List.hd expr_list
				else
					List.fold_left (fun x y -> ADD (x, y)) (List.hd expr_list) (List.tl expr_list) in
			galculator extended_list))
	| INTEGRAL (a, b, f) -> (
		if (a > b) then galculator (MUL (INT (-1), INTEGRAL (b, a, f)))
		else (
...
(*
let _ = print_endline (string_of_float (galculator (SIGMA (INT 1, INT 1, SUB(MUL(X, X), INT 1)))))
let _ = print_endline (string_of_float (galculator (INTEGRAL (REAL 1., REAL 1., SUB(MUL(X, X), INT 1)))))
