...
let rec mathemadiga expr =
	...
	| INTEGRAL(a, b, c) ->
		let aVal = mathemadiga a in
		let bVal = mathemadiga b in 
	(	match ((aVal>(bVal+.0.1)), (aVal<(bVal-.0.1))) with
		| (true, true) -> raise ERROR 
		| (true, false) -> (-0.1) *. (mathemadiga (x_transfer (REAL aVal) c)) +. mathemadiga (INTEGRAL(ADD(a,REAL (-0.1)), b, c))
		| (false, false) -> (bVal-.aVal) *. (mathemadiga (x_transfer (REAL aVal) c))
		| (false, true) -> 0.1 *. (mathemadiga (x_transfer (REAL aVal) c)) +. mathemadiga (INTEGRAL(ADD(a,REAL 0.1), b, c)) 
	)
