...
let rec galculator input = 
	...
	and
  	...
  	doIntegral (here2, there2, formula) =
		if here2 +. 0.1 > there2 then 0.0 
  		else (0.1 *. (computing (here2, formula))) +. doIntegral (here2 +. 0.1, there2, formula)
  	in
  	match input with
	...
	| INTEGRAL (first, second, third) -> if galculator first < galculator second then doIntegral (galculator first, galculator second, third)
						else -1.0 *. doIntegral (galculator second, galculator first, third)
;;
