...
=fun tenv e ty -> 
  ...
  | LETREC (f,x,e1,e2) -> 
    ...
    let ntenv = tenv_extend (f,TyFun (a1,a2)) (tenv_extend (x,a1) tenv) in
    (gen_equations ntenv e1 a2) @ (gen_equations ntenv e2 ty)
  | PROC (x,e) -> 
    let a1 = fresh_tyvar () in
    let a2 = fresh_tyvar () in 
...
= fun t1 t2 subst -> 
  ...
  | (TyVar a,t) -> 
    (match t with
    | TyInt | TyBool -> subst_extend a t subst
    | TyFun (t1,t2) -> if (t1=(TyVar a) || t2=(TyVar a)) then raise TypeError else subst_extend a t subst
    | TyVar b -> if a=b then subst else subst_extend a t subst)
  | (t,TyVar a) -> unify t2 t1 subst
  | (TyFun (t1,t2),TyFun (t1',t2')) -> 
    let s' = unify t1 t1' subst in
    let s'' = unify (subst_apply t2 s') (subst_apply t2' s') s' in
    s''
  | _ -> raise TypeError
