...
let rec natadd ((x:nat),(y:nat)):nat = match y with
  |ZERO -> x
  |SUCC value -> natadd ((SUCC x), value)

let rec natmul ((x:nat),(y:nat)):nat = match x with
  |ZERO -> ZERO
  |SUCC value1 -> natadd (y, natmul(value1,y))
