...
let rec diffTimesList l x = match l with
  | [] -> []
  | hd::tl -> match hd with
    | Const c -> if c=0 then [Const 0] else hd::(diffTimesList tl x)
    | Var v -> if v=x then diffTimesList tl x else [Const 0]
    | Power (v, p) -> if v=x then [Const p; Power (v, p-1)]@(diffTimesList tl x) else hd::(diffTimesList tl x)
    | Times l2 -> (diffTimesList l2 x)@(diffTimesList tl x);;
    
let rec diffSumList l x = match l with
  | [] -> []
  | hd::tl -> match hd with
    | Const c -> diffSumList tl x
    | Var v -> if v = x then (Const 1)::(diffSumList tl x) else diffSumList tl x
    | Power (v, p) -> if v =x then (Times [Const p; Power(v, p-1)])::(diffSumList tl x) else diffSumList tl x 
