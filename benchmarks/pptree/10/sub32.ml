...
let rec make_treelst tourna n =
	...
	|NODE (a,b) -> append (make_blank (((get_2 (get_level a b)) + (List.length (List.hd (List.tl (make_treelst a (get_level a b))))) - 1)/2)) (append ["|"] (make_blank (((get_2 (get_level a b)) + (List.length (List.hd (List.tl (make_treelst a (get_level a b))))) - 1)/2)))
      
   :: append_3 (remove_tail (List.hd (make_treelst a (get_level a b))) (((List.length (List.hd (List.tl (make_treelst a (get_level a b))))) /2)+1))
