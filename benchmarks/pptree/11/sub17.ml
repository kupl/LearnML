		 	...
		 	(match t1 with 
			 	A _ -> raise Error
		   		| L -> 
					(match t2 with 
					 A _ -> raise Error
					| L -> (makeTlist (t@[t1; t2]) ([-2;1;-1;1]::tlist) height (r+1) (c+1))
					| N (_, _) -> (makeTlist (t@[t1; t2]) ([-2;0;-1;1]::tlist) height (r+1) (c+1)))
				| N (_, _) ->
			   		(match t2 with
					 A _ -> raise Error
				    | L -> (makeTlist (t@[t1; t2]) ([-2;1;-1;0]::tlist) height (r+1) (c+1))
				    | N (_, _) -> (makeTlist (t@[t1; t2]) ([-2;0;-1;0]::tlist) height (r+1) (c+1)))
			))
		 ...
		 | N (t1, t2) ->
			(match t1 with
			    ...
			    | L ->
					(match t2 with
					 A _ -> raise Error
					| L -> (makeTlist (t@[t1; t2]) ([-2;1;-1;1]::tlist) height r (c+1))
					| N (_, _) -> (makeTlist (t@[t1; t2]) ([-2;0;-1;1]::tlist) height r (c+1)))
				| N (_, _) ->
				    (match t2 with
					 A _-> raise Error
				    | L -> (makeTlist (t@[t1; t2]) ([-2;1;-1;0]::tlist) height r (c+1))
				    | N (_, _) -> (makeTlist (t@[t1; t2]) ([-2;0;-1;0]::tlist) height r (c+1)))
			))
	     ...
	     | N (t1, t2) ->
		     (match t1 with
			    ...
			    | L ->
				    (match t2 with
					 A _ -> raise Error
				    | L -> (makeTlist (t@[t1; t2]) (([-2;1;-1;1]@head)::tail) height (r+1) (c+1))
					| N (_, _) -> (makeTlist (t@[t1; t2]) (([-2;0;-1;1]@head)::tail) height (r+1) (c+1)))
				| N (_, _) ->
				    (match t2 with
					 	A _ -> raise Error
					   | L -> (makeTlist (t@[t1; t2]) (([-2;1;-1;0]@head)::tail) height (r+1) (c+1))
					   | N (_, _) -> (makeTlist (t@[t1; t2]) (([-2;0;-1;0]@head)::tail) height (r+1) (c+1)))
			)))
	     ...
	     | N (t1, t2) ->
		     (match t1 with
			     ...
			     | L ->
				    (match t2 with
					 A _ -> raise Error
				    | L -> (makeTlist (t@[t1; t2]) (([-2;1;-1;1]@head)::tail) height r (c+1))
				    | N (_, _) -> (makeTlist (t@[t1; t2]) (([-2;0;-1;1]@head)::tail) height r (c+1)))
				 | N (_, _) ->
				    (match t2 with
					 A _ -> raise Error
				   | L -> (makeTlist (t@[t1; t2]) (([-2;1;-1;0]@head)::tail) height r (c+1))
				   | N (_, _) -> (makeTlist (t@[t1; t2]) (([-2;0;-1;0]@head)::tail) height r (c+1)))
			 )))
		)
in
...
let rec mkstrL l h r =
	let blank = 2.**(float (h-r+1))-.1.
	in
	match l with
	[] -> ""
	| head::tail -> 
		(if (head=(-2)) then ((mkstr " " "" blank)^(mkstrL tail h r))
		 else if (head=(-1)) then ((mkstr "-" "" blank)^(mkstrL tail h r))
		 else if (head=0 || head=1) then ("|"^(mkstrL tail h r))
		 else ((mkstr " " "" (2.**(float (h-r))*.((float head)-.1.)))^"|"^(mkstr " " "" (2.**(float (h-r))*.((float head)-.1.)))^(mkstr " " "" blank)^(mkstrL (tlN l head) h r)))
in
