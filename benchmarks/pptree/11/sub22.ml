...
let rec pptree tour =
	...
	(* n층에서 leaf일때 *)
	let middle n = 
		let snum n = ((magic (n+2))-1)/2 in
		(space (snum n)) ^ "|" ^ (space (snum n))
	in
	(* n층에서 leaf가 아닐때 *)
	let full n = 
		"|" ^ (hyphon (magic (n+1))) ^ "|"
	...
	let floor2 (n, d) = 
		...
		else (space (magic n)) ^ floor (n, magic (d - n)) ^ (space (magic n))
	in	
	...
	let rec replace (d, cnt, number, lst, special) =
		let location = (String.length (floor (d-(cnt+1), (number-(power 2 cnt)) * 2 ))) in
		let ang = 
		replace_elem (lst,
					  replace_string (  location, 
										String.length (middle (d-(cnt+1))), 
