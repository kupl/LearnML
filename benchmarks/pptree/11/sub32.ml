...
let pptree tr =
	...
	let rec merge_sublist input = (* merge_sublist : 두 리스트를 받아 새로운 string 리스트를 생성 *)
		...
		|(cd, dl, al::ll, dr, ar::lr)	-> 	
									...
									let ch str = 
										let str_leng = (String.length str) in
										if str_leng > (String.length "|") 
											then 
												if (String.get str 0) = '|' 
													then ("|" ^ " " ^ (String.sub str 2 (str_leng-2)))
												else if (String.get str (str_leng-1)) = '|'  
													then ((String.sub str 0 (str_leng-2)) ^ " " ^ "|")
												else str
										else str in
									if dl = 1 && dr = 1 
