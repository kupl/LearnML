type exp =
  ...
  let rec input_var : exp -> string list -> string list = 
  fun e lst ->
    match e with 
    |V(v)->lst 
  ...
  ;;
  let rec input_exp : exp->string list -> string list 
  = fun e lst -> 
    match e with 
    |V(v)->lst@[v] 
    |P(v,e1) -> input_exp e1 lst 
    |C(e1,e2)->input_exp e1 (input_exp e2 lst) 
  ;;
  ...
  = fun exp -> 
    let l2=input_var exp [] in 
    let l1 = input_exp exp [] in
    match exp with 
    |V(v) -> false 
