...
let rec premerge (lh, rh) =                     (* heap * heap -> heap *)
	...
	| (NODE(_,lval,lhltree,lhrtree), (NODE(_,rval,rhltree,rhrtree))) ->
		if (lval < rval) then NODE(0, lval, lhltree, premerge(lhrtree, rh))
		else NODE(0, rval, rhltree, premerge(lh, rhrtree))
