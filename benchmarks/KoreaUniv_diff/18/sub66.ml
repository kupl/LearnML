  ...
  match exp with
    ...
    |Times t ->
      let rec reTime lst = 
        match lst with
          |[] -> false
          |hd::tl -> (isExist hd x) || reTime tl
      in reTime t
    | Sum s ->
      let rec reSum lst = 
        match lst with
          |[] -> false
          |hd::tl -> (isExist hd x) || reSum tl
      in reSum s;;
...
= fun (exp, x) -> match exp with
    ...
    |Times t -> if not(isExist (Times t) x) then Const 0 else
      let rec reTime lst = 
        match lst with
          |[] -> []
          |hd::tl -> match hd with
      ...
      in Times (reTime t)
    |Sum s -> if not(isExist (Sum s) x) then Const 0 else
      let rec reSum lst = 
        match lst with
          |[] -> []
          |hd::tl -> (diff (hd, x))::(reSum tl)
      in Sum (reSum s);;
      
diff (Sum [Power ("x", 2); Times [Const 2; Var "x"]; Const 1], "y");;
diff (Sum [Times [Var "y"; Var "x"]; Const 2], "y");;