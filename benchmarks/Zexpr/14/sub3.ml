...
module Zexpr: ZEXPR = 
  struct
    ...
    let rec change(env, expr) = 
      ...
      |_ -> raise (Error "This variable is not defined in current environment")
    ...
    let rec eval_cal (env, expr) =
      ...
      |VAR a -> 
	  (match env with
	  |LIST [] -> raise (Error "This variable is not defined in current environment")
	  |LIST ((id, exp)::tail) -> 
	      (if id = a then exp
