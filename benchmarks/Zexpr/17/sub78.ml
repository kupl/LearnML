...
struct 
  ...
  let emptyEnv = []
  let rec getValue (env, i) = 
    match env with
      [] -> raise (Error "FreeVariable")
    | h::t -> (if ((fst h) = i) then snd h
                                else getValue (t, i))
  ...
  let rec eval ((env : environment), (e : expr)) : value =
    ...
    | MAX lst -> (match lst with
                  ...
                  | h::t -> (match t with
                              [] -> eval (env, h)
                            | _ -> (Pervasives.max (eval (env, h)) (eval (env, MAX t)))))
    | VAR i -> getValue (env, i)
    | LET (i, e1, e2) -> eval ((i, eval(env, e1))::env, e2)

  let rec print_value i = 
    print_endline(Pervasives.string_of_int(i))
