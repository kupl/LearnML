...
struct
  ...
  | MAX [] -> 0
  | MAX (hd::[]) -> eval (env,hd)
  | MAX (hd::tl) -> if (eval (env,hd)>eval (env,MAX tl)) then (eval (env,hd)) else (eval (env,MAX tl))
  | VAR vid -> (match env with 
  		| [] -> raise (Error "FreeVariable")
		| ((id,nb)::[]) -> if(id=vid) then nb else raise (Error "FreeVariable")
		| ((id,nb)::tl) -> if(id=vid) then nb else eval (tl,VAR vid)
		)
